...
Agent B — 核心引擎
參考文件: 06-核心程式-告警管理.md, 10-審查與修正報告.md

NOTE: call_log 表結構（12 欄）必須與 Agent C 的 web/models.py 完全一致，
      不可自行增減欄位。含 routed_group TEXT DEFAULT '' 欄位。
"""
...
class AlertManager:
    ...
    def is_duplicate(self, alert_key: str) -> bool:
        ...
        [v2 修正] 同時使用記憶體快取（快）和 SQLite（持久化），
        確保服務重啟後仍能正確去重。

        Args:
            alert_key: 告警唯一識別（alertName + resourceName）
    ...
    def call_with_escalation(
        ...
        Args:
            wav_path: WAV 語音檔路徑
            targets: 撥號目標列表 [{"name":..., "number":..., "priority":...}]
            alert_data: 原始告警資料（供日誌記錄）
            routed_group: 匹配的路由規則名稱（供日誌記錄）
        """
    ...
    def _init_db(self):
        ...
        IMPORTANT: call_log 表結構（12 欄）必須與 Agent C 的 web/models.py
        完全一致，不可自行增減欄位。
        """
